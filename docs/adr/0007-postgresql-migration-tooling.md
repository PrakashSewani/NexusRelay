...
- Use private schema `nexusrelay` for application objects and private schema `nexusrelay_migration` for Atlas revision history. The schema owner owns `nexusrelay`; the migration login owns `nexusrelay_migration` and the Atlas revision table directly. `PUBLIC` has no access to either schema.
- Scope the Atlas connection to `search_path=nexusrelay` and set `--revisions-schema nexusrelay_migration`. This allows trusted initialization to pre-create both empty private schemas without making Atlas treat unrelated database state as dirty; normal deployment still prohibits `--allow-dirty` and baselining.
...
- The exceptional graph-upgrade procedure is represented by a reviewed SQL artifact and approved request bound by SHA-256. A short-lived cluster-admin runner executes it transactionally before Atlas and atomically publishes a protected external evidence bundle containing the request, SQL, exact before/after graph verification, and sanitized execution output. The repository ships no standing or sample mutation SQL.
- The reusable exact PostgreSQL 18 role/edge verifier gates Atlas in Compose and is reused by initialization, restore, and graph-upgrade evidence. Any missing/extra NexusRelay role or edge, attribute drift, edge-option drift, or unsupported PostgreSQL major blocks migration.
