...
func main() {
	...
	observability.InstallBootstrapLogger("gateway", os.Stderr)
...
func writeVersion(arguments []string, output io.Writer) bool {
	return buildinfo.WriteVersion(arguments, output, "nexusrelay-gateway")
}

func run(ctx context.Context) error {
	settings, err := config.LoadGateway()
	if err != nil {
		return err
	}
	telemetry, err := observability.New(ctx, "gateway", settings.Shared.Version, settings.Observability, os.Stdout)
	if err != nil {
		return err
	}
